...
const BlogSearch = ({
  ...
  const [productSelectCategoryData, setProductSelectCategoryData] = useState(
    []
  );
  ...
  useEffect(() => {
    setProductSelectCategoryData(
      Array?.isArray(categoryData)
        ? categoryData?.filter((dt) => dt?.slug === categorySlug)
        : []
    );
  }, [categorySlug]);
  ...
  const searchProductData = async (year, makes, models, bodys, category) => {
    document?.getElementById("dialogclose")?.click();
    setUpdateLoader(true);
    setMobileBody(false);
    let ccSlug = null;
    if (category || category_slug) {
      ccSlug = category || category_slug;
      setcategorySlug(category || category_slug);
    } else {
      ccSlug = category || categoryData[0]?.slug;
      setcategorySlug(category || categoryData[0]?.slug);
    }
    if (year && makes && models && bodys) {
      ...
      setLoadingSearch(true);
      const ccdata = Array?.isArray(categoryData)
        ? categoryData?.filter((dt) => dt?.slug === ccSlug)
        : [];
      const data = {
        category: ccSlug,
        year: year,
  ...
  return (
    <>
      ...
      {Boolean(skeletonLoading) ? (
        <>
          <section className="form-wrapper pb-5">
            <div className="container px-0">
              <div
                className="form-inline mx-1 cover-search row overflow-hidden rounded-0 py-4"
                aria-hidden="true"
              >
                {/* <div
                ...
                <div className="col-12">
                  <div className="row pt-xl-0 pt-3">
                    <div className="col-xl-3 mb-xl-0 mb-3 col-12 placeholder-glow">
                      <h4 className="w-100 placeholder mb-0 py-3"></h4>
                    </div>
                    <div className="col-xl-3 mb-xl-0 mb-3 col-12 placeholder-glow">
                      <h4 className="w-100 placeholder mb-0 py-3"></h4>
                    </div>
        ...
        <>
          <div
            className={`${storeData?.language?.code != "en" ? "sc" : ""} pb-5`}
            id="scrollSearch"
          >
            <section className="form-wrapper">
              <div className="container px-0">
                <div className="form-inline cover-search rounded-0 pb-2 pt-xl-1 pt-3">
                  {/* <h3
      ...
      {
        Boolean(productData?.result) && (
          <>
            {Boolean(productData?.result?.length) && (
              <Products
                breadcrumb={false}
                slugs={{
                ...
                cartID={categoryData}
                isBodyExist={productSelectCategoryData[0]?.is_body === false}
                isFilterExist={productSelectCategoryData?.[0]?.is_filter}
                // istilesExist={true}
                meta_description={
                  productSelectCategoryData?.[0]?.details?.meta_description
                }
              />
            )}
          </>
